...
def connect_to_server(soc):
    ...
    """
    try:
        print(f"{Colors.BOLD}{Colors.CYAN}⛏️  MINERADOR duinoCoin FPGA KINTEX 7 v1{Colors.RESET} {Colors.YELLOW}by @frenow{Colors.RESET}")
...
while True:
    ...
    try:
        ...
        while True:
            ...
            if (int(difficulty) > 21000000): # minerador fpga v1 só irá funcionar com dificuldade até 2.100.000.000
                print(f'{Colors.WARNING}⚠️  [{current_time()}]{Colors.RESET} Dificuldade muito alta: {difficulty} (máximo suportado: 21.0M)')
                break
            ...
            result_msg = f"{nonce},{int(hashrate)},fpga_kintex7_miner"
